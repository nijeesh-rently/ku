    ...
    module Commands
      ...
      class Database < Base
        ...
        def database_host(envs)
          envs['DATABASE_HOST'] || envs['DB_HOST'] || envs['RDS_HOST']
        end

        def database_name(envs)
          envs['DATABASE_NAME'] || envs['DB_NAME'] || envs['RDS_DB_NAME']
        end

        def database_password(envs)
          envs['DATABASE_PASSWORD'] || envs['DB_PASSWORD'] || envs['RDS_PASSWORD']
        end

        def database_port(envs)
          envs['DATABASE_PORT'] || envs['DB_PORT'] || envs['RDS_PORT'] || 5432
        end

        def database_username(envs)
          envs['DATABASE_USERNAME'] || envs['DB_USERNAME'] || envs['RDS_USERNAME']
        end
      end
    end
  end
